...
# --- Portable skills from template (not project-overlay) ---
...
# --- MCP audit scripts from Hub install ---
...
# --- Template rules from reference project (exclude project-specific + Hub SoT) ---
$SkipRules = @(
...
$CursorDocs = @(
    "MCP_ROUTER.md", "MCP_SETUP.md", "MCP_PLAYBOOKS.md",
    "RULES_INDEX.md", "README.md", "INSTALL_OTHER_DEVICE.md"
)
foreach ($doc in $CursorDocs) {
...
# INSTALL_ALL from ESTI if present (master checklist for all projects)
...
# --- Commands: merge template + ESTI extras ---
$tplCmd = Join-Path $TemplateProject ".cursor\commands"
$estiCmd = "C:\Cursor\ESTI\.cursor\commands"
$dstCmd = Join-Path $Repo ".cursor\commands"
New-Item -ItemType Directory -Force -Path $dstCmd | Out-Null
foreach ($srcDir in @($tplCmd, $estiCmd)) {
...
foreach ($stub in @("24-always-mcp-tool-router.mdc", "26-always-no-webfetch.mdc")) {
    ...
    if (Test-Path -LiteralPath $active) {
        $off = Join-Path $legacyDir ($stub + ".off")
        Move-Item -Force -LiteralPath $active -Destination $off
        Write-Host "MOVED $stub -> _legacy/$($stub).off"
    }
}

# --- ESTI project rules + LURV docs (no secrets) ---
$estiRules = @(
...
foreach ($r in $estiRules) {
    $src = "C:\Cursor\ESTI\.cursor\rules\$r"
    if (Test-Path -LiteralPath $src) {
        Copy-Tracked -Src $src -Dst (Join-Path $Repo ".cursor\rules\$r")
    }
}
foreach ($doc in @("ESTI_LURV_FROM_CHAT.md", "ESTI_POST_lurv.md", "EXTENT_CONTRACTORS.md")) {
    $src = Join-Path $Repo "docs\$doc"
    # keep docs already prepared in repo; refresh POST-lurv from ESTI if present
}
$posti = Get-ChildItem "C:\Cursor\ESTI\Extent" -Directory -ErrorAction SilentlyContinue | Where-Object { <#
.SYNOPSIS
  Push Hub agent-pack + project template updates into a local CursorSettings git clone.
.PARAMETER Repo
  Path to CursorSettings repo (default: %TEMP%\CursorSettings).
...
Write-Host "  git push"
Write-Host "Also run: .\Sync-1cAgentPack.ps1 in cursor-addons\install (projects already PASS if no Hub edits)."
.Name -like "*HTTP*" -and <#
.SYNOPSIS
  Push Hub agent-pack + project template updates into a local CursorSettings git clone.
.PARAMETER Repo
...
Write-Host "  git push"
Write-Host "Also run: .\Sync-1cAgentPack.ps1 in cursor-addons\install (projects already PASS if no Hub edits)."
.Name -match "LURV|ЛУРВ" } | Select-Object -First 1
if ($posti) {
    $postSrc = Join-Path $posti.FullName "POST-lurv.md"
    if (Test-Path -LiteralPath $postSrc) {
        Copy-Tracked -Src $postSrc -Dst (Join-Path $Repo "docs\ESTI_POST_lurv.md")
    }
}
# Contractors template from UT if missing in repo

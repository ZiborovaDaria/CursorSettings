...
1c-cfe-manage borrow               — borrow objects to modify (never hand-write Adopted XML)
1c-cfe-manage patch                 — generate interceptors (or write BSL after verify in typical)
1c-cfe-manage validate              — check XML structure
cfe-verify-interceptors.ps1         — verify &Перед/&После/&ИзменениеИКонтроль vs base (skill cfe-load-hang-recovery)
cfe-recover.ps1 -Action Diagnose    — IB shell + files; before LoadConfigFromFiles
LoadConfigFromFiles / designer      — only after Diagnose OK
1c-cfe-manage diff -Mode A          — review changes overview
1c-cfe-manage diff -Mode B          — check transfer status
```

**Hang at 100% on load:** see skill `.cursor/skills/cfe-load-hang-recovery/SKILL.md` (broken IB shell, invalid interceptors, manual borrow XML with wrong `InternalInfo` TypeIds).

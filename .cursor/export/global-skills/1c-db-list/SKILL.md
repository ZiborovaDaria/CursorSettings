...
6. Если файл `.v8-project.json` не найден — для проекта SB используй `infobasesettings.md`:
   - ИБ: `C:\Users\Daria\Documents\InfoBase11`
   - Без `/N` и `/P` (аутентификация отключена)
   - Предложи создать `.v8-project.json` из шаблона в корне репозитория
